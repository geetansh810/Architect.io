...
const languageOf = (path) => {
  ...
  if (path === 'Dockerfile' || path === '.dockerignore') return 'docker';
...
export default function CodePreview({ nodes, edges }) {
  ...
  // Authenticated sessions preview the REAL enterprise project generated by
  // the server; the demo canvas falls back to the local approximation.
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        const res = await api('/generate/preview', {
          method: 'POST',
          body: JSON.stringify(parseToBackendPayload()),
        });
        if (!res.ok) return; // validation errors etc. → fall back to local preview
        const data = await res.json();
        if (!cancelled) {
          setServerFiles(Object.entries(data).map(([path, content]) => ({
            path, content, language: languageOf(path),
          })));
        }
          ...
          <div className="w-48 border-r border-[var(--border-main)] overflow-y-auto py-2 bg-[var(--bg-sidebar)]">
            {generatedFiles.map((file, i) => (
              ...
              >
                ...
                <span className="truncate" title={file.path}>{file.path.replace(/^src\//, '')}</span>
              </button>
            ))}
          </div>
        )}
